...
// localStorage.setItem("#textArea", storeContent);
localStorage.setItem("miltime" + time, currentStorage)
});

// function matchHours to work with the for loop to help determine the time and asscoiated color (currently Broken)

// attempting to set local storage as well. 
function matchHours (){
...
for (var i = 9; i < 18; i++){
    ...
    $("#textArea"+ i).removeClass("past").addClass("present")
    
} else (i > currentHour)
    $("#textArea"+ i).removeClass("present").removeClass("past").addClass("future")
